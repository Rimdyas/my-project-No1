...
let registerBtn = document.getElementById('registration-icon');
...
function addToCart(event) {
    ...
    // Додаємо товар до кошика, але тільки якщо він ще не доданий
...
class ShoppingCart {
   ...
   // Додавання товару до кошика (з перевіркою, щоб не додавати більше одного разу)
addItem(item) {
    if (this.items[item.title]) {
        alert('Ця послуга вже додана в кошик.');
        return; // Якщо товар вже є, виводимо повідомлення і не додаємо повторно
    } else {
        this.items[item.title] = item; // Якщо товару немає в кошику, додаємо його
        this.items[item.title].quantity = 1;
    }
    this.updateCounter(); // Оновлюємо лічильник товарів
    this.saveCartToCookies(); // Зберігаємо оновлений кошик
}
